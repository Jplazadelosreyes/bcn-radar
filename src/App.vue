...
// ═══ Zonas administrativas (distritos / barrios) como capa de mapa ═══
const zoneOn = ref({})       // level -> visible
const zoneStatus = ref({})   // level -> 'loading'|'error'
...
async function toggleZone(level) {
  ...
  if (!on) { ids.forEach(id => map.getLayer(id) && map.setLayoutProperty(id, 'visibility', 'none')); return }
  if (map.getSource(src)) { ids.forEach(id => map.getLayer(id) && map.setLayoutProperty(id, 'visibility', 'visible')); return }
  ...
  try {
    ...
    // generateId permite seleccionar la zona clicada con feature-state
    map.addSource(src, { type: 'geojson', data: gj, generateId: true, attribution: 'Límites: Ajuntament de Barcelona (CartoBCN)' })
    // Relleno: transparente por defecto, se tiñe al seleccionar (y captura el clic)
    map.addLayer({
    ...
    map.on('click', `${src}-fill`, (e) => {
      ...
      if (prev === f.id) { zoneSel[src] = null; return }
  ...
  } catch (e) { console.warn('zonas', e); zoneStatus.value = { ...zoneStatus.value, [level]: 'error' }; zoneOn.value = { ...zoneOn.value, [level]: false } }
}

// ═══ Renta media por sección censal (coropleta sociodemográfica) ═══
...
async function toggleRenta() {
  ...
  if (!on) { ids.forEach(id => map.getLayer(id) && map.setLayoutProperty(id, 'visibility', 'none')); return }
  if (map.getSource('renta')) { ids.forEach(id => map.getLayer(id) && map.setLayoutProperty(id, 'visibility', 'visible')); return }
  ...
  } catch (e) { console.warn('renta', e); rentaStatus.value = 'error'; rentaOn.value = false }
}
      ...
      <aside class="sidebar" :class="{ open: sidebarOpen, full: sheetFull }">
        ...
        <SectionCard ico="🗺️" title="Zonas administrativas" :open="!!cardOpen['zonas']" body-class="capas-body" @toggle="toggleCard('zonas')">
            ...
            <label class="capas-layer">
              ...
              <span class="capas-layer-body">
                <span class="capas-layer-t">Distritos
            ...
            <label class="capas-layer">
              ...
              <span class="capas-layer-body">
                <span class="capas-layer-t">Barrios
            ...
            <label class="capas-layer">
              ...
              <span class="capas-layer-body">
                <span class="capas-layer-t">Secciones censales
                ...
                <span class="capas-layer-h">1.068 secciones — la unidad del INE para renta y demografía (código al hacer zoom).</span>
              </span>
            </label>
            <label class="capas-layer">
              <input type="checkbox" :checked="rentaOn" @change="toggleRenta()">
              <span class="capas-layer-body">
                <span class="capas-layer-t">Renta media (mapa de calor)
